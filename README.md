...
## Следующий web-first MVP scope

- email magic link авторизация;
- production-safe browser session на базе bearer/JWT или cookie-сессии;
- home/settings UI для in-app reminders: просроченные и ближайшие проверки;
- настройка `usageCycleDays`, `nextCheckAt` и `reminderEnabled` в UI;
- continued browser smoke без Telegram credentials;
...
- email magic link auth endpoints, токены входа и отправку писем;
- production-safe browser auth без dev auth;
- пользовательскую настройку циклов проверки и включения/выключения
  in-app reminders для товаров, категорий и наборов в UI;
...
До реализации email magic link локальный режим авторизации разрешен только при
`NODE_ENV=development`. В production он должен быть выключен.
