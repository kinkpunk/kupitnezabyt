...
- расширенное двухаккаунтное e2e-покрытие collaboration beta и удобство
  приглашения пользователей, которые еще не вошли в сервис;
...
Совместные списки можно показывать пользователям как beta-функцию после
production-like двухаккаунтного smoke-теста. Текущая модель шарит весь активный
список/workspace, а не отдельный набор товаров. Владелец может пригласить
участника по email, отозвать ожидающее приглашение, удалить доступ и передать
владение.

Перед широким включением нужно закрыть Slice 32 из
`docs/IMPLEMENTATION_ROADMAP.md`: приглашения для email, которые еще не вошли в
сервис, двухаккаунтный smoke на production, и понятные beta-ограничения в UX.
