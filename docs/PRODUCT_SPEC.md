...
2. Для каждого товара выбирает статус: есть, мало или купить.
3. Товары со статусом `NEED_BUY` автоматически попадают в список покупок.
...
1. Если есть хотя бы один товар `NEED_BUY`, категория получает `NEED_BUY`.
2. Иначе, если есть `LOW`, категория получает `ATTENTION`.
3. Иначе категория получает `OK`.
4. Архивные и приостановленные товары не участвуют в расчете.
...
Список автоматически включает активные товары со статусом `NEED_BUY`.

Требования:

1. Позиции отображаются в порядке добавления (`createdAt` по возрастанию).
...
4. Пользователь выбирает `Есть`, `Мало` или `Купить`.
5. Статус сохраняется через общую backend-логику переходов.
6. После последнего товара сессия завершается.
7. Показывается количество проверенных товаров и покупок в списке.
...
У тебя 4 товара в списке покупок.
```
...
1. Позиции в порядке добавления.
2. Группировку по категориям.
3. Ручные позиции.
4. Действие `Отметить купленным`.
5. Действие очистки завершенных позиций.

### 9.7. Проверка

На экране одновременно отображается одна карточка товара, прогресс сессии и три действия изменения статуса.
...
1. `NEED_BUY` создает или обновляет одну открытую позицию для товара.
2. Повторный запрос не создает дубль.
3. `mark-bought` завершает позицию и переводит товар в `IN_STOCK`.
4. При ручном переводе из `NEED_BUY` в `IN_STOCK`, соответствующая незавершенная позиция также завершается.

Операция обновления товара и списка покупок выполняется транзакционно.

### 13.3. Агрегирование категории

Порядок статусов при агрегировании:

```text
NEED_BUY > LOW > IN_STOCK
```
...
6. `NEED_BUY` синхронизируется со списком покупок без дублей.
